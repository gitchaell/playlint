import { ChevronDown, History, Plus, RotateCcw, Settings2 } from 'lucide-react';
		...
		<aside className='w-full h-full bg-zinc-50 dark:bg-zinc-950 flex flex-col p-4 z-40 overflow-hidden border-r border-zinc-200 dark:border-zinc-800'>
			<div className='mb-6 px-2 flex items-center justify-between'>
				<div className='flex items-center gap-3 w-full'>
					...
					<div className='truncate flex-1'>
						<h2 className='text-zinc-900 dark:text-white font-black text-sm uppercase tracking-tight leading-none font-title truncate'>
							{t('linter.config')}
						</h2>
			...
			<div className='mt-auto pt-4 border-t border-zinc-200 dark:border-zinc-800 space-y-1 shrink-0'>
				<button
					...
					className='w-full flex items-center gap-3 px-3 py-2 text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300 transition-all text-xs truncate'
				>
				...
				<button
					onClick={() => alert('Feature coming soon')}
					className='w-full mt-4 bg-zinc-900 dark:bg-white text-white dark:text-black font-bold py-2.5 rounded-lg text-xs hover:bg-zinc-800 dark:hover:bg-zinc-200 border border-zinc-800 transition-colors font-title truncate'
				>
