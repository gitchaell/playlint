										...
										{rule.options && rule.options.length > 0 ? (
											...
											<div className='mb-6 w-fit relative'>
												<input
													...
													onChange={(e) =>
														onSettingChange(path, parseInt(e.target.value, 10))
													}
	...
	return (
		...
		>
			<summary className='flex justify-between items-center p-6 cursor-pointer hover:bg-zinc-50 dark:hover:bg-zinc-900/50 transition-colors list-none [&::-webkit-details-marker]:hidden select-none'>
				<div>
					...
					<div className='flex items-center gap-3'>
						...
						{description && (
							<span className='text-xs text-zinc-500 font-sans hidden sm:inline-block truncate max-w-sm'>
								{description}
							</span>
						)}
					</div>
				...
				<div className='flex items-center gap-4 shrink-0'>
					...
					<div
						className={`p-1.5 rounded-full transition-colors ${isOpen ? 'bg-zinc-100 dark:bg-zinc-800 text-zinc-900 dark:text-white' : 'text-zinc-500'}`}
					>
						<ChevronDown
			...
			<div className='border-t border-zinc-200 dark:border-zinc-800/30'>
				{description && (
					<p className='px-6 pt-4 text-xs text-zinc-500 dark:text-zinc-400 font-sans sm:hidden leading-relaxed'>
						{description}
					</p>
				)}
				{children}
