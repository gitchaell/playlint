				...
				<div className='w-full max-w-2xl bg-zinc-950 rounded-xl flex flex-col shadow-2xl border border-zinc-800 font-sans max-h-[90vh]'>
					...
					<div className='flex items-center justify-between w-full px-6 h-16 border-b border-zinc-800 shrink-0'>
						<div className='flex items-center gap-3'>
							...
							<h2 className='font-bold text-lg tracking-tight text-white font-title'>
								{t('modal.title')}
							</h2>
						</div>
						<button
					...
					<div className='flex-1 overflow-y-auto p-6 space-y-8'>
						<section className='space-y-4'>
							<h3 className='font-title font-bold text-xs uppercase tracking-widest text-zinc-500'>
								{t('modal.selectEngine')}
							</h3>
								...
								{tools.map((tool) => {
									...
									return (
										...
										>
											...
											<span
												className={`font-title font-bold text-sm ${isActive ? 'text-white' : 'text-zinc-500'}`}
											>
												{tool.name}
											</span>
										</button>
						...
						<section className='grid grid-cols-1 sm:grid-cols-2 gap-6'>
							<div className='space-y-2'>
								<label className='font-title font-bold text-xs uppercase tracking-widest text-zinc-500 ml-1'>
									{t('modal.configName')}
								</label>
						...
						<section className='space-y-4'>
							<h3 className='font-title font-bold text-xs uppercase tracking-widest text-zinc-500 ml-1'>
								{t('modal.starterTemplate')}
							</h3>
							<div className='space-y-3'>
								...
								>
									...
									<div className='flex-1'>
										<div className='flex justify-between items-center mb-1'>
											<span
												className={`font-title font-bold ${selectedTemplate === 'Standard Recommended' ? 'text-white' : 'text-zinc-300'}`}
											>
												{t('modal.template.standard')}
											</span>
											<div
												className={`w-4 h-4 rounded-full border-2 ${selectedTemplate === 'Standard Recommended' ? 'border-white bg-white' : 'border-zinc-700'}`}
											></div>
										</div>
										<p className='text-sm text-zinc-500 leading-relaxed'>
											{t('modal.template.standardDesc')}
										</p>
									</div>
								</div>
								...
								>
									...
									<div className='flex-1'>
										<div className='flex justify-between items-center mb-1'>
											<span
												className={`font-title font-bold ${selectedTemplate === 'Minimalist' ? 'text-white' : 'text-zinc-300'}`}
											>
												{t('modal.template.minimalist')}
											</span>
											<div
												className={`w-4 h-4 rounded-full border-2 ${selectedTemplate === 'Minimalist' ? 'border-white bg-white' : 'border-zinc-700'}`}
											></div>
										</div>
										<p className='text-sm text-zinc-500 leading-relaxed'>
											{t('modal.template.minimalistDesc')}
										</p>
									</div>
								</div>
							</div>
